...
<template>
  ...
  <div class="group relative flex min-w-0 flex-col rounded-lg border border-line bg-white p-3 shadow-[0_2px_8px_rgba(0,0,0,.09)] transition duration-200 hover:z-10 hover:border-brand/20 hover:shadow-[0_10px_32px_rgba(0,0,0,.18)] sm:p-4">
    <NuxtLink :to="`/urun/${product.slug}/`" class="block rounded focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand/40">
      ...
      <!-- line-clamp rather than a fixed height with overflow hidden: long
           titles were being sliced mid-word with no ellipsis, so there was
      ...
      <div class="relative aspect-square overflow-hidden rounded">
        ...
        <!-- Over the photo, not over the category line: at the card's top-left
             it was landing on top of the text. -->
        <span
          v-if="product.discount"
          class="absolute right-0 top-0 rounded bg-price px-2 py-0.5 text-[11px] font-bold text-white"
        >
          -%{{ product.discount }}
        </span>
